...
class SocketHandler(websocket.WebSocketHandler):
    ...
    def open(self):
        ...
        logging.info('WebSocket opened, clients: %s' % len(
    ...
    def on_close(self):
        ...
        logging.info('WebSocket closed, clients: %s' % len(
            self.websocket_clients))
...
class TasksHandler(tornado.web.RequestHandler):
    ...
    def get(self, task_id, action, task_run_id):
        ...
        if task_id == '' and action == '':
            tasks = self.task_controller.get_task_list(
                with_history=True, task_run_limit=20)
            result = json.dumps(tasks)
        elif task_id != '':
            task = self.task_controller.get_task_by_id(
                task_id, with_extra_info=True)
            ...
            if action == '':
                result = json.dumps({
                    'config': task,
                    'runs': self.task_controller.get_task_runs_for_task_id(
                        task_id)
                })
                ...
                detailed_task_run = \
                    self.task_controller.get_detailed_task_run_info(
                        task_id, task_run_id)

                if not detailed_task_run:
                    self.raise_404('Task run %s/%s not found' % (
                        task_id, task_run_id))
                else:
                ...
                task_run_stdout = \
                    self.task_controller.get_stdout_for_task_run_id(
                        task_id, task_run_id)

                if not task_run_stdout:
                    self.raise_404('Stdout %s/%s not found' % (
                        task_id, task_run_id))
                else:
                ...
                task_run_stderr = \
                    self.task_controller.get_stderr_for_task_run_id(
                        task_id, task_run_id)
                if not task_run_stderr:
                    self.raise_404('Stderr for %s/%s not found' % (
                        task_id, task_run_id))
                else:
    ...
    def post(self, task_id, action, task_run_id):
        ...
        if task_id == '' and action == '':
            ...
            if not is_valid_config:
                self.raise_500(
                    'Cannot update task with config: %s' % json.dumps(config))
            else:
    ...
    def delete(self, task_id, action, task_run_id):
        ...
        if task_id == '' and action == '':
            ...
            if not task:
                self.raise_500(
                    'Cannot delete task with config: %s' % json.dumps(config))
            else:
                result = 'Success'
        self.set_header('Access-Control-Allow-Origin', '*')
        self.write(result)
...
def main():
    ...
    define(
        "notifier_config", type=str, help="notifier json config",
        default=os.path.dirname(
            os.path.realpath(__file__)) + "/../notifier.json")
    define(
        "notify_to", default="logger", type=str, help="notifications channel")
    define(
        "notifier_host", default="hostname", type=str,
        help="host for links in notifications")
    ...
    default_wi_config_path = os.path.dirname(
        os.path.realpath(__file__)) + '/ereb-wi/default_config.js'
    ...
    logging.info(
        "Starting EREB on http://{}:{}".format('0.0.0.0', options.port))
    ...
    application = tornado.web.Application([
        (
            ...
            dict(task_controller=task_controller)
        ),
        (
        ...
        (
            r'/ws', SocketHandler, dict(task_controller=task_controller,
                                        websocket_clients=websocket_clients)
        ),
        (
            r"/(.*)", tornado.web.StaticFileHandler,
            {
                "path": os.path.dirname(
                    os.path.realpath(__file__)) + "/ereb-wi",
    ...
    signal.signal(signal.SIGTERM, partial(
        shutdown, task_controller.shutdown_tasks))
