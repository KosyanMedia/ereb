...
class Notifier:
    def __init__(
            self, notifier_config, notify_to='logger', websocket_clients=None,
            host='hostname', port=8888):
    ...
    def error(self, link, message, task_run):
        if self.cmd:
            if self.notify_to.startswith('slack_api'):
                info = task_run.log_info(
                    lines_count=self.cmd.get('include_strings', 2))
                slack_payload = {
                        'attachments': [{
                            'pretext': 'Failed task',
                            'title': info['task_id'],
                            ...
                            'text': self.cmd.get(
                                'text',
                                'Failed task {task_id}').format(**info),
                            'fallback': message,
                            'short': True,
                            'color': 'danger',
                ...
                response = requests.post(
                    self.cmd['webhook_url'], json=slack_payload)

                if response.status_code != 200:
                    logging.error(
                        'Notifications Slack API error: %s' % response.text)
            else:
                if "http" in self.cmd:
                ...
                notifier_cmd = self.cmd.replace('%s', "{0}\n{1}".format(
                    link, message))
                subprocess.Popen(
                    notifier_cmd, shell=True, stdout=subprocess.PIPE)
        else:
            logging.warning("Notifications are turned off")

    def send_failed_task_run(self, task_run):
        link = "http://{0}:{1}/#/tasks/{2}/runs/{3}".format(
            self.hostname, self.port, task_run.task_id, task_run.id)
