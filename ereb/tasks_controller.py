...
class TaskController:
    def __init__(self, tasks_dir="etc", history_dir="./var",
                 notifier_config=None, notify_to='logger',
                 notifier_host='hostname', websocket_clients=None, port=8888):
        ...
        self.notifier = Notifier(notifier_config=notifier_config or {},
                                 notify_to=notify_to,
                                 websocket_clients=websocket_clients or [],
                                 host=notifier_host,
                                 port=port)
        ...
        self.process_checking_loop = PeriodicCallback(
            self.check_processes, 10000)
    ...
    def check_processes(self):
        ...
        for currently_running_task in self.history_storage.\
            ...
            if 'pid' in task_run.state:
                if task_run.state['pid'] != 'None':
                    if psutil.pid_exists(task_run.state['pid']):
                        logging.info(
                            'Task %s with run %s is alive', task_run.task_id,
                            task_run.id)
                    else:
                        logging.info(
                            'Task %s with run %s is dead already; finalized',
                    ...
                    logging.info(
                        'Task %s with run %s is in unknown state, no pid; '
                        'finalized', task_run.task_id, task_run.id)
                    self.history_storage.finalize_task_run(task_run)
                    # FIXME: Prolly it's now working. DO SOMETHING
    ...
    def get_dashboard_info(self):
        dashboard = {
            'recent_fails': self.history_storage.get_recent_failed_task_runs(
                20)}
    ...
    def get_detailed_history_for_task_id(self, task_id, limit=20):
        return self.history_storage.get_detailed_history_for_task_id(
            task_id, limit)

    def get_detailed_task_run_info(self, task_id, task_run_id):
        return self.history_storage.get_detailed_task_run_info(
            task_id, task_run_id)

    def get_stdout_for_task_run_id(self, task_id, task_run_id):
        detailed_task = self.history_storage.get_detailed_task_run_info(
            task_id, task_run_id, content_size_limit=False)
    ...
    def run_task_by_task_id(self, task_id):
        ...
        self.task_scheduler.run_task_by_name_and_cmd(
            task['name'], task['cmd'], task.get('timeout', -1))
